...
type InViewState = readonly [React.RefObject<HTMLDivElement | null>, boolean];
...
const TRUST_POINTS = ["Развёртывание за 24 ч", "99.9% Uptime SLA"];

const STATS = [
  { value: "< 3 с", label: "Время обнаружения угрозы", icon: Zap, delay: 0 },
  { value: "98.7%", label: "Точность классификации ИИ", icon: Brain, delay: 80 },
  { value: "360°", label: "Покрытие периметра", icon: Target, delay: 160 },
  { value: "50+", label: "Типов БПЛА в базе ИИ", icon: Cpu, delay: 240 },
];

const FEATURE_CARDS = [
  { title: "ИИ-планирование защиты", description: "Алгоритм анализирует геометрию объекта, существующие устройства и угрозы — и автоматически предлагает оптимальную расстановку сенсоров.", icon: Brain, delay: 0 },
  { title: "Мониторинг в реальном времени", description: "Радарные сигналы, RF-спектр и видео с PTZ-камер объединены в единый экран оператора. Угроза классифицируется до появления в зоне риска.", icon: Activity, delay: 70 },
  { title: "Аналитика и отчётность", description: "Динамика инцидентов, uptime оборудования, эффективность операторов — в виде наглядных графиков и экспортируемых отчётов.", icon: BarChart3, delay: 140 },
  { title: "3D-конфигуратор", description: "Интерактивная трёхмерная расстановка устройств на модели предприятия с визуализацией зон покрытия и готовыми сценариями защиты.", icon: ScanLine, delay: 210 },
  { title: "ИИ-рекомендации", description: "Система непрерывно анализирует инциденты и выдаёт рекомендации: где добавить устройство, как изменить сценарий, какие зоны уязвимы.", icon: TrendingUp, delay: 280 },
  { title: "Управление командой", description: "Роли операторов, привязка к объектам, SMS-уведомления при тревоге. Полный контроль над доступами и реакцией персонала.", icon: Users, delay: 350 },
];

const AI_METRICS = [
  { value: "−40%", label: "Снижение бюджета на защиту\nза счёт точечного размещения устройств" },
  { value: "+2.3×", label: "Рост эффективности обнаружения\nпосле 90 дней обучения ИИ" },
  { value: "0", label: "Слепых зон после применения\nрекомендаций по конфигурации" },
];
...
const HOW_STEPS = [
  { num: "01", title: "Сенсоры засекают объект", description: "Радар, RF-детектор и PTZ-камеры одновременно фиксируют движение. ИИ сопоставляет сигналы и классифицирует тип БПЛА за миллисекунды.", delay: 0 },
  { num: "02", title: "Оператор принимает решение", description: "На экране Центра тревог — данные с датчиков и видеопоток. Один клик — тревога объявлена, SMS разослан всем сотрудникам.", delay: 120 },
  { num: "03", title: "ИИ анализирует и улучшает", description: "После инцидента система обновляет рекомендации по конфигурации, корректирует бюджет защиты и дополняет базу знаний об угрозах.", delay: 240 },
];

const DEMO_EVENTS = [
  { id: "rf-detector-3", src: "RF-детектор #3", zone: "B4", level: "high", time: "14:32:07" },
  { id: "radar-1", src: "Радар #1", zone: "B4", level: "medium", time: "14:32:05" },
  { id: "camera-7", src: "Камера #7", zone: "B3", level: "low", time: "14:31:58" },
];

const BRANDS = ["Droneshield", "Robin Radar", "Dedrone", "Squarehead", "Aaronia", "Hikvision"];
const CTA_NOTES = ["Без обязательств", "Бесплатная консультация", "Конфиденциальность данных"];
...
function HeroThemeToggle() {
  ...
  return (
    <button
      ...
      className="h-8 w-8 rounded-lg border border-white/12 dark:border-white/12 bg-white/8 hover:bg-white/14 dark:bg-white/5 dark:hover:bg-white/10 grid place-items-center text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors cursor-pointer"
...
function useInView(threshold = 0.15): InViewState {
...
function Pill({ children, color = "sky" }: { children: ReactNode; color?: string }) {
  return (
    <span className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-1 text-[11px] font-semibold uppercase tracking-widest ${PILL_COLORS[color] ?? PILL_COLORS.sky}`}>
      {children}
    </span>
  );
}
...
function FeatureCard({ icon, title, description, delay = 0, inView }: {
  icon: ReactNode; title: string; description: string; delay?: number; inView: boolean;
}) {
  return (
    <div
      className="group relative rounded-2xl border border-slate-200/60 dark:border-white/8 bg-white/70 dark:bg-white/4 p-6 backdrop-blur-sm transition-colors duration-500 hover:border-sky-400/50 dark:hover:border-sky-500/30 hover:shadow-lg hover:shadow-sky-500/5"
      style={{
        opacity: inView ? 1 : 0,
    ...
    >
      ...
      <div className="mb-4 h-11 w-11 rounded-xl bg-sky-500/10 border border-sky-500/20 dark:border-sky-500/20 grid place-items-center text-sky-600 dark:text-sky-400 group-hover:bg-sky-500/16 transition-colors duration-300">
        {icon}
      </div>
...
function StatCard({ value, label, icon, delay, inView }: {
  value: string; label: string; icon: ReactNode; delay: number; inView: boolean;
}) {
  return (
    <div
...
function FloatingDemoCta({ show }: { show: boolean }) {
  return (
    <div
      className="fixed right-5 top-1/2 -translate-y-1/2 z-40"
      style={{
        opacity: show ? 1 : 0,
        transform: show ? "translateY(-50%) translateX(0)" : "translateY(-50%) translateX(calc(100% + 20px))",
    ...
    >
      <a
        href="#cta"
        className="group flex flex-col items-center gap-2 rounded-2xl px-4 py-4 bg-sky-500 hover:bg-sky-400 shadow-lg shadow-sky-500/30 hover:shadow-sky-500/50 transition-colors duration-200 cursor-pointer"
      >
...
function HomeHeroSection({ dark, heroCtaRef }: { dark: boolean; heroCtaRef: RefObject<HTMLDivElement | null> }) {
  ...
  return (
    <section className="relative min-h-screen flex flex-col items-center justify-center px-6 pt-20 pb-24 text-center overflow-hidden">
      ...
      <div ref={sectionRef} className="relative z-10 flex flex-col items-center gap-6 max-w-3xl">
        ...
        <div style={{ opacity: inView ? 1 : 0, transform: inView ? "translateY(0)" : "translateY(20px)", transition: "opacity 0.7s ease 100ms, transform 0.7s ease 100ms" }}>
        ...
        <p
          ...
          style={{ opacity: inView ? 1 : 0, transform: inView ? "translateY(0)" : "translateY(18px)", transition: "opacity 0.7s ease 200ms, transform 0.7s ease 200ms" }}
        >
        ...
        <div
          ...
          style={{ opacity: inView ? 1 : 0, transform: inView ? "translateY(0)" : "translateY(16px)", transition: "opacity 0.7s ease 300ms, transform 0.7s ease 300ms" }}
        >
          <Link
            href="/prototype"
        ...
        <div
          ...
          style={{ opacity: inView ? 1 : 0, transition: "opacity 0.7s ease 450ms" }}
        >
          {TRUST_POINTS.map((t) => (
    ...
    <section id="stats" className="relative py-20 px-6">
      <div className="max-w-5xl mx-auto">
        <div ref={sectionRef} className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {STATS.map(({ value, label, icon: Icon, delay }) => (
            <StatCard key={label} value={value} label={label} icon={<Icon className="h-5 w-5" />} delay={delay} inView={inView} />
          ))}
        </div>
      </div>
    </section>
      ...
      <div className="max-w-5xl mx-auto relative">
        <div ref={sectionRef}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {FEATURE_CARDS.map(({ title, description, icon: Icon, delay }) => (
              <FeatureCard
                key={title}
                inView={inView}
                delay={delay}
      ...
      <div className="max-w-5xl mx-auto relative">
        <div ref={sectionRef}>
          ...
          <div
            ...
            style={{ opacity: inView ? 1 : 0, transform: inView ? "translateY(0)" : "translateY(16px)", transition: "opacity 0.6s ease 100ms, transform 0.6s ease 100ms" }}
          >
            <div className="flex flex-wrap justify-center gap-10 md:gap-16">
              {AI_METRICS.map(({ value, label }) => (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {AI_IMPROVEMENTS.map(({ title, description, accent, icon: Icon, iconClassName, delay }) => (
              <AIImprovCard
                key={title}
                inView={inView}
                delay={delay}
        ...
        <div ref={sectionRef}>
          ...
          <div className="grid md:grid-cols-2 gap-12 items-center">
            <div className="flex flex-col gap-10">
              {HOW_STEPS.map(({ num, title, description, delay }) => (
                <StepCard key={num} num={num} title={title} description={description} delay={delay} inView={inView} />
              ))}
            </div>
            <AlarmCenterMockup inView={inView} />
          </div>
        </div>
      </div>
    </section>
...
function AlarmCenterMockup({ inView }: { inView: boolean }) {
  return (
    <div
      ...
      style={{ opacity: inView ? 1 : 0, transform: inView ? "translateX(0)" : "translateX(30px)", transition: "opacity 0.7s ease 150ms, transform 0.7s ease 150ms" }}
    >
      ...
      {DEMO_EVENTS.map((ev) => (
        <div key={ev.id} className="flex items-center gap-3 rounded-xl border border-slate-200/60 dark:border-white/5 bg-slate-50/80 dark:bg-white/3 px-4 py-2.5 mb-2">
...
function HomeCtaSection() {
  ...
  return (
    <section id="cta" className="relative py-28 px-6 overflow-hidden">
      ...
      <div className="max-w-2xl mx-auto text-center relative" ref={sectionRef}>
        <div style={{ opacity: inView ? 1 : 0, transform: inView ? "translateY(0)" : "translateY(24px)", transition: "opacity 0.7s ease, transform 0.7s ease" }}>
          ...
          {!submitted ? (
            <form onSubmit={handleSubmit} className="mt-8 flex flex-col sm:flex-row gap-3 max-w-md mx-auto">
              <label className="sr-only" htmlFor="home-demo-email">Email для демо</label>
              <input
                id="home-demo-email"
                type="email"
                required
                ...
                className="flex-1 rounded-xl border border-slate-200 dark:border-white/12 bg-white dark:bg-white/6 px-4 py-3 text-[14px] text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none focus:border-sky-400 dark:focus:border-sky-500/60 transition-colors"
              />
              <button
                type="submit"
                className="flex items-center justify-center gap-2 rounded-xl bg-sky-500 hover:bg-sky-400 px-6 py-3 text-[14px] font-semibold text-white transition-colors shadow-lg shadow-sky-500/25 hover:shadow-sky-500/40 whitespace-nowrap"
              >
                Запросить демо
                <ArrowRight className="h-4 w-4" />
              </button>
...
export function HomePage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#080d1a] text-slate-900 dark:text-white overflow-x-hidden font-sans transition-colors duration-300">
      ...
      <FloatingDemoCta show={showFloatingCta} />
      <HomeNav />
      <HomeHeroSection dark={dark} heroCtaRef={heroCta} />
      <HomeStatsSection />
      <HomeFeaturesSection />
      <HomeAiSection />
      <HomeHowSection />
