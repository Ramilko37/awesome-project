...
- переходы `Карта`, `Расчёт`, `Сценарии`;
- общий светлый workspace-фон.

Состояние сценарного прототипа сейчас хранится в `useDefenseStudioStore.view`. Поэтому ссылка `Сценарии` ведёт на `/prototype?view=scenario-modeling`, но ставит `view = "drilldown"`. Старый прямой вход `/prototype?view=3d` сохраняется как alias. Карта активна, когда route `/prototype` и `view !== "drilldown"`.
...
- `FacilityDrilldown` для прототипа Модуля сценарного моделирования.

`FacilityDrilldown` возвращает старую R3F/Three.js-сцену как отдельный прототип будущего сценарного направления. Этот режим не является основным рабочим контуром GIS MVP и использует `useDefenseStudioStore` для demo-runtime/local placements.
...
Глобальная кнопка `3D` из нижней панели удалена; переход в сценарный 3D-прототип остаётся отдельной кнопкой `Сценарии` в левом rail и mobile nav.
